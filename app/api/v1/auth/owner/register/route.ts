...
export async function POST(request: Request) {
  try {
    const { phone, password, name, business_name } = await request.json();

    if (!phone || !password || !name || !business_name) {
    ...
    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
      phone,
      password,
      phone_confirm: true, // Auto-confirm phone for password login
